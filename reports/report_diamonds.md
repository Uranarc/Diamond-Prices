...
The analysis was conducted in Python 3 using the following core libraries:

- `numpy` — numerical computation
- `pandas` — data manipulation and profiling
- `matplotlib`, `seaborn` — visualisation
- `statsmodels` — OLS regression (smf.ols) and VIF computation
- `sklearn` — `train_test_split`, `StandardScaler`, `PCA`, `mean_squared_error`
- `scipy` — skewness and kurtosis statistics

A complete list of versions is specified in `requirements.txt` at the repository root.
